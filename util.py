...
def create_vectorstore(pdf_docs):
    ...
    embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")  # Get the embedding function
    vectorstore = FAISS.from_texts(texts=text_chunks, embedding=embeddings)
    return vectorstore


# Get response from llm of user asked question
def get_llm_response(llm, prompt, question, behaviour, role, number_q, difficulty):
    document_chain = create_stuff_documents_chain(llm, prompt)
    retrieval_chain = create_retrieval_chain(st.session_state.vector_store.as_retriever(), document_chain)
    response = retrieval_chain.invoke({'input': question, 'behaviour':behaviour, 'role': role, 'number_q': number_q, 'difficulty': difficulty})
    return response
